...
namespace tasks.repository
{
	public abstract class AbstractRepository<ID, T> : ICrudRepository<ID, T> where T : HasId<ID>
	{
		...
		public virtual void update(int integer, T entity)
		{
			//if (!items.ContainsKey(int.TryParse(Request. integer)))
			//	items.Add(entity.Id, entity);
			//else throw new RepositoryException("ID " + integer + " does not exist");
		}
	}
